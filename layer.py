    ...
    def edge_detect2(self):
        for y in range(self.height):
            for x in range(self.width):
                ...
                next_grayscale = grayscale
                if x < self.width-1:
                    new_pixel = self.get_pixel(x+1, y)
                    next_grayscale = math.floor((new_pixel[0]+new_pixel[1] + new_pixel[2])/3)

                difference = next_grayscale - grayscale
                absolute_diff = abs(difference)

                
                self.set_pixel(x,y, (int(absolute_diff), int(absolute_diff), int(absolute_diff)))
    ...
    def edge_detect(self):
        for y in range(self.height):
            for x in range(self.width):
                ...
                if x != 0:
                    previous_pixel = self.get_pixel(x-1, y)
                    previous_grayscale = math.floor((previous_pixel[0]+previous_pixel[1] + previous_pixel[2])/3)

                difference = previous_grayscale - grayscale
                absolute_diff = abs(difference)
